...
class CustomerAddressSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        errors = {}
        # Require street, city, state, phone
        for field in ["street", "city", "state", "phone"]:
            value = data.get(field)
            if not value:
                errors[field] = f"{field.capitalize()} is required."

        phone = data.get("phone")
        if phone and not re.match(r"^\+?\d{9,13}$", phone):
            errors["phone"] = "Phone must be 9-13 digits, optionally starting with '+'."
